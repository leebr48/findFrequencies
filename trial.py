...
def scipy_coo_to_petsc(mat_coo):
    """
    Converts a scipy.sparse.coo_matrix directly into a petsc4py.PETSc.Mat.
    """
    # 1. Convert COO to CSR (fast internal SciPy operation)
    ...
    # 3. Create the PETSc Matrix using the CSR arrays directly
    petsc_mat = PETSc.Mat().createAIJ(size=mat_csr.shape, csr=(indptr, indices, data))
    petsc_mat.setUp()
    petsc_mat.assemble()
...
if __name__ == "__main__":
    ...
    print("Loading SciPy matrices...")
    A_scipy = FieldBendingMatrix('.').load_matrix()
    B_scipy = InertiaMatrix('.').load_matrix()
    
    # 2. Convert directly to PETSc format
    print("Converting to PETSc Mat format...")
    ...
    print(f"Extraction complete. Found {len(found_evals)} eigenvalues strictly within the real bounds.")
