...
# ====================================================================================
# 4. SOLVER: CISS WITHOUT MUMPS + Options Fix
# ====================================================================================
def solve_eigenproblem(A_dense, B_dense, comm):
    ...
    # ----------------------------------------------------------------------
    # THE FIX: Use PETSc Options to bypass missing slepc4py Python methods
    # ----------------------------------------------------------------------
    opts = PETSc.Options()
    
    # 1. Force CISS to keep MPI communicators intact
    opts.setValue('-eps_ciss_partitions', 1) 
    
    # 2. Force CISS to use the primary ST object (Thanks to your C docs find!)
    ...
    # Configure Iterative Preconditioner (BJACOBI/GMRES) on the primary ST
    # Because we set '-eps_ciss_usest', we NO LONGER need to iterate over getCISSKSPs()!
    st = eps.getST()
    ...
    comm.Barrier()
    if rank == 0: print(f"--- EXECUTING SETUP ---", flush=True)
    eps.setUp()
